...
export const FactoryRunKeyPartsSchema = Schema.Tuple(
  Schema.String,
  Schema.String,
)
export type FactoryRunKeyParts = Schema.Schema.Type<
  typeof FactoryRunKeyPartsSchema
>
...
export type FactoryRunKeyString = Schema.Schema.Type<
  typeof FactoryRunKeyStringSchema
>

export const PermissionResolutionKeyPartsSchema = Schema.Tuple(
  Schema.String,
  Schema.String,
)
export type PermissionResolutionKeyParts = Schema.Schema.Type<
  typeof PermissionResolutionKeyPartsSchema
>
...
const isCanonicalPermissionResolutionKey = (encoded: string): boolean => {
  const decoded = Schema.decodeUnknownEither(PermissionResolutionKeySchema)(encoded)
  if (decoded._tag === "Left") return false
  return Schema.encodeSync(PermissionResolutionKeySchema)(decoded.right) === encoded
}

export const PermissionResolutionKeyStringSchema = Schema.String.pipe(
  Schema.filter(encoded =>
    isCanonicalPermissionResolutionKey(encoded)
      ? undefined
      : "PermissionResolutionKey must be the canonical encoded tuple"),
)
export type PermissionResolutionKeyString = Schema.Schema.Type<
  typeof PermissionResolutionKeyStringSchema
>
