...
`Schema.decodeUnknownEffect` / transform-style decoding, and use `@effect/cli`
...
- A shared launch-config schema for MCP server declarations, so CLI arguments
  and durable RuntimeContext launch ingress normalize through the same contract
  before adapter-specific lowering.
