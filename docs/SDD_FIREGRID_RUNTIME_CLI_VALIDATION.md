...
`firegrid-runtime-process.SCENARIOS.15` keeps the private scenarios package in a
normal `src` layout: shared row contracts and inspection live at the package
source root, input emitters live under `src/emitters`, and app-owned receivers
with their scenario tests live under `src/receivers`.
