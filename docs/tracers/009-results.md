...
Required actions live under `packages/runtime/src/required-action/**`, with durable record schemas exported from `packages/protocol/src/required-action/**`. Stale `control-plane` and `data-plane` runtime directories were read for existing patterns only; materialization/runtime directory moves remain outside Lane C scope.

## Ownership Resolution

Required-action protocol schema extraction is complete for `firegrid-required-actions.RECORDS.4` and `firegrid-required-actions.BOUNDARY.5`. Required-action topology is intentionally not integrated into `FiregridRuntimeHostLive` as bespoke host methods. `firegrid-required-actions.WORKFLOW.7`, `firegrid-required-actions.BOUNDARY.6`, and `firegrid-platform-invariants.AUTHORITY.8` require a future generic reactive workflow/operator substrate before required actions become the model for workflow-backed tools.
