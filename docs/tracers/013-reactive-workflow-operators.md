...
No replacement required-action operator is introduced by this cleanup. Required
actions retain their current domain facts and workflow semantics while the
workflow/operator architecture moves to the generic `effect-durable-operators`
package.
...
- Required-action scenarios may compose the production workflow engine and
  `RequiredActionsLive` for required-action semantics, but they must not add a
  required-action-specific operator or mini-root.
...
- `packages/runtime/src/required-action/required-action.test.ts`
